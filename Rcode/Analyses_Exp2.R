...
data.e
# A tibble: 177 x 20
#         Trial TestDay Date                Fish GroupSize Speed RewardArm ArmChosen Response OriginalTimeTaken NewTravelTime NewTotalTime DecisionZoneTime
# <dbl>   <dbl> <dttm>              <int>     <int> <int> <chr>     <chr>        <int>             <dbl>         <dbl>        <dbl>            <dbl>
#   1   1.00    1.00 2016-02-24 00:00:00     1         1     1 R         R                1              8.00          2.75         8.26            0.701
# ...
# ...
# ... with 167 more rows, and 7 more variables: GroupSizeFinalOut <int>, NFishHoldingArm <dbl>, NFishCorrectArm <dbl>, NFishIncorrectArm <dbl>,
#   SubjectGroupSize <dbl>, Ngroups <int>, SubjectRank <dbl>

# --- Factors: GroupSize x Speed
table(data.e[,c(5,6)])
#           Speed
# GroupSize  1 10
# 1         24 23
...
summary(data.e)
# --- NA's generated by fish that never made a decision (coded as having a Response = 999)
...
# Movement bias? - No
# -----------------------------------
table(data.e2$ArmChosen) # fish that made a decision
# L  R 
# 86 78 
...
mTA1.0 = lme(I(log(NewTotalTime)) ~ Speed * GroupSize,random = ~1|Fish, data=data.e)
anova(mTA1.0)
...
mTime2ActDiagnostics <- grid.arrange(p1, p2, p3, p4, p5,nrow=2,top = textGrob("Time 2 Act"))
...
mTA4.0 = lme(I(log(NewTotalTime)) ~ Speed * GroupSize,random = ~1|Fish, data=data.e2)
anova(mTA4.0)
#                 numDF denDF   F-value p-value
...
TAmean.s <- data.eL %>%
  group_by(GroupSize) %>% 
  summarize(Time.mean = mean(NewTotalTime), Time.sd = sd(NewTotalTime)) %>%
  mutate(LTime.mean = log(Time.mean),LGroupSize = log(GroupSize))

# -- on medians
TAmed.s <- data.eL %>%
  group_by(GroupSize) %>% 
  summarize(Time.med = median(NewTotalTime), Time.mad = mad(NewTotalTime)) %>%
...
rm(data.eL)
      ...
      TA.s <- data.e %>%
        group_by(GroupSize) %>% 
        summarize(Time.mean = mean(NewTotalTime),Time.sd = sd(NewTotalTime), Time.cv = sd(NewTotalTime)/mean(NewTotalTime),
                  Time.med = median(NewTotalTime), Time.mad = mad(NewTotalTime)) %>%
      ...
      # ---- main figure
      Fig3a_TotalTimeXShoalSizeBoxplots <-  ggplot(data.e, aes(x = factor(GroupSize), y = NewTotalTime)) +
        #  gpTotalTimeXShoalSizeBoxplots_NoGrid <-  ggplot(data.e, aes(x = factor(GroupSize), y = NewTotalTime)) +
      ...
      tiff("./Figures/Fig3a.tiff",w = 3,h = 3,units="in",res=300)
      ...
      # A tibble: 2 x 7
      # G15Outliers = subset(data.e, GroupSize == 15 & NewTotalTime > 750,)
      # G15Outliers
      #     Trial TestDay Date                Fish  GroupSize Speed NewTotalTime
...
# ==================================================
      ...
      ggplot(data.e2, aes(GroupSize,DecisionZoneTime,color=factor(Speed))) + geom_point() + geom_jitter()
      ggplot(subset(data.e2,DecisionZoneTime < 25), aes(GroupSize,DecisionZoneTime,color=factor(Speed))) + geom_point() + geom_jitter()
      
      
      DTxR.s <- data.e2 %>%
        group_by(GroupSize,Response) %>% 
        summarize(DT.m = mean(DecisionZoneTime), DT.se = sd(DecisionZoneTime)/sqrt(n())) 
      
      ggplot(data = DTxR.s, aes(x = GroupSize, y = DT.m, fill = factor(Response))) +
      ...
      DTxR2.s <- data.e2 %>% subset(DecisionZoneTime <=25) %>%
        group_by(GroupSize,Response) %>% 
        summarize(DT.m = mean(DecisionZoneTime), DT.se = sd(DecisionZoneTime)/sqrt(n())) 
      
      ggplot(data = DTxR2.s, aes(x = GroupSize, y = DT.m, fill = factor(Response))) +
        geom_errorbar(aes(ymin=DT.m - DT.se, ymax = DT.m + DT.se), width = 0.2,size=0.5) +
        geom_point(size=2,shape=21)
...
# mTD1.0 = lme(I(log(DecisionZoneTime)) ~ Speed * GroupSize,random = ~1|Fish, data=mdata)
mTD1.0e3 = lme(I(log(DecisionZoneTime)) ~ Speed * GroupSize,random = ~1|Fish, data=mdata)
...
# GroupSize       1   137  0.33284  0.5649
  ...
  # -----------------------
  # Steps: 
  # 1. diagnostics on data.e2 model for DecisionZoneTimes
  ...
  # mDecisionTimeDiagnosticsData2 <- grid.arrange(p1, p2, p3, p4, p5,nrow=2,top = textGrob("Decision time: data.e2"))
  mDecisionTimeDiagnosticsData3 <- grid.arrange(p1, p2, p3, p4, p5,nrow=2,top = textGrob("Decision time: data.e3"))
  ...
  ggplot(data.e2, aes(DecisionZoneTime,Response)) + geom_point()
  
  mDAxS = glmer(Response ~ DecisionZoneTime + (1|Fish), data = data.e2, family='binomial', nAGQ = 15)
  
  summary(mDAxS)
  # Fixed effects:
  #                   Estimate Std. Error z value Pr(>|z|)  
  # (Intercept)      0.004134   0.393238   0.010   0.9916  
  # DecisionZoneTime 0.384473   0.185226   2.076   0.0379 *
  
  # -----------------------
  # Decision Speed ~ Response correlations x Shoal size 
  # -----------------------
  ...
  cor.test(c1$DecisionZoneTime, c1$Response) # N.S, P = 0.07, r = 0.30 
  cor.test(c5$DecisionZoneTime, c5$Response) # N.S, P = 0.07, r = 0.28
  cor.test(c10$DecisionZoneTime, c10$Response) # N.S, P = 0.60, r = 0.09
  cor.test(c15$DecisionZoneTime, c15$Response) # N.S, P = 0.71, r = 0.06
      ...
      Fig3b_TDxResponse <- ggplot(data = pDTxR.s, aes(x = GroupSize, y = DT.m, fill = factor(Decision))) +
        geom_errorbar(aes(ymin=DT.m - DT.se, ymax = DT.m + DT.se), width = 0.2,size=0.5) +
        geom_point(size=2,shape=21) +
      ...
      ggsave("./Figs/Fig3b_TDxResponse.tiff", Fig3b_TDxResponse, width = 3, height = 3.25, dpi = 300)
...
# --- visualization 
      ggplot(data.e2, aes(GroupSize,Response,color=factor(Speed))) + geom_point() + geom_jitter()
      
      
      # Decision accuracy x speed summary for plotting
      DA.s <- data.e2 %>%
        group_by(GroupSize, Speed) %>% 
        summarize(R.m = mean(Response), R.se = sd(Response)/sqrt(n())) 
...
mDA1 <- glmer(Response ~ Speed * GroupSize + (1|Fish), data = data.e2, family='binomial', nAGQ = 15)
summary(mDA1) 
# Fixed effects:
#                   Estimate Std. Error z value Pr(>|z|)  
